...
from model_utils import tensorized, sort_by_lengths, cal_lstm_crf_loss, save_model
from model.bilstm import BiLSTM


class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, args):
        """功能：对LSTM的模型进行训练与测试
           参数:
            vocab_size:词典大小
            out_size:标注种类
        ...
        if os.path.isfile(args.save_model + '/bilstm_crf.ckpt'):
            self.model.load_state_dict(torch.load(args.save_model + '/bilstm_crf.ckpt'))
        self.cal_loss_func = cal_lstm_crf_loss

        # 加载训练参数：
        self.epoches = args.max_epoch
        self.print_step = args.step_check
        self.lr = args.lr
    ...
    def train(self, word_lists, tag_lists,
        ...
        for e in range(self.epoches):
            self.step = 0
            losses = 0.
            print('Epoch [{}/{}]'.format(e+1, self.epoches))
            for ind in range(0, len(word_lists), B):
                ...
                if self.step % self.args.step_check == 0:
                    total_step = (len(word_lists) // B + 1)
                    print("Epoch {}, step/total_step: {}/{} {:.2f}% Loss:{:.4f}".format(
                        e+1, self.step, total_step,
            ...
            print("Epoch {}, Val Loss:{:.4f}".format(e+1, val_loss))
    ...
    def validate(self, dev_word_lists, dev_tag_lists, word2id, tag2id):
        ...
        with torch.no_grad():
            ...
            if val_loss < self._best_val_loss:
                print("保存模型...")
                save_model(self.model, self.args.save_model + '/bilstm_crf.ckpt')
